...
func NewJobExperienceSubCommand(parentCommand *cobra.Command) {
	generateCmd := &cmdgenerate.GenerateSubCommand{
		...
		GenerateFunc:          GenerateJobExperience,
		UseCollectionManifest: true,
		CollectionKind:        "Profile",
		UseWorkloadManifest:   true,
		WorkloadKind:          "JobExperience",
	}

	generateCmd.Setup()
}

// GenerateJobExperience runs the logic to generate child resources for a
// JobExperience workload.
func GenerateJobExperience(g *cmdgenerate.GenerateSubCommand) error {
	...
	generateFuncMap := map[string]generateFunc{
		"v1alpha1": v1alpha1jobexperience.GenerateForCLI,
